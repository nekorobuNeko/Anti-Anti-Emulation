...
static BOOL(WINAPI* TrueGlobalMemoryStatusEx)(LPMEMORYSTATUSEX lpBuffer) = GlobalMemoryStatusEx;
HWND WINAPI newGlobalMemoryStatusEx(LPMEMORYSTATUSEX lpBuffer) {
	long long a = 2147483648;
	puts("hooked!!");
	TrueGlobalMemoryStatusEx(lpBuffer);
	lpBuffer->ullTotalPhys = a;
	return 0;
}
...
BOOL WINAPI DllMain(HINSTANCE hinst, DWORD dwReason, LPVOID reserved)
{
	...
	if (dwReason == DLL_PROCESS_ATTACH) {
		...
		DetourAttach(&(PVOID&)TrueGlobalMemoryStatusEx, newGlobalMemoryStatusEx);
		...
		DetourTransactionBegin();
		DetourUpdateThread(GetCurrentThread());
		DetourDetach(&(PVOID&)TrueGlobalMemoryStatusEx, newGlobalMemoryStatusEx);
		error = DetourTransactionCommit();
